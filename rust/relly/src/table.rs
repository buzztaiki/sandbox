...
impl UniqueIndex {
    ...
    pub fn insert(&self, bufmgr: &mut BufferPoolManager, key: &[u8], record: &[&[u8]]) -> Result<()> {
        let btree = BTree::new(self.meta_page_id);
        let mut skey = vec![];
        tuple::encode(self.skey_cols.iter().map(|&index| record[index].as_ref()),
                      &mut skey);
        btree.insert(bufmgr, &skey, key)?;
        Ok(())
    }
}
