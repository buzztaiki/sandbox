// NOTE: crate はパッケージ。なので use crate::List はこのパッケージの List というような意味になる。
use crate::List::*;
// NOTE: crate を付けなくても use できる模様。
// use List::*;

enum List {
    // NOTE: Box は値をヒープに起くための仕組みらしい。see https://doc.rust-jp.rs/rust-by-example-ja/std/box.html
